...
- `commands.config` (default `false`) enables `/config` (reads/writes `zovsironclaw.json`).
...
- Overrides apply immediately to new config reads, but do **not** write to `zovsironclaw.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`zovsironclaw.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
