...
```bash
zovsironclaw onboard --auth-choice cloudflare-ai-gateway-api-key
```
...
If the Gateway runs as a daemon (launchd/systemd), make sure `CLOUDFLARE_AI_GATEWAY_API_KEY` is available to that process (for example, in `~/.zovsironclaw/.env` or via `env.shellEnv`).
