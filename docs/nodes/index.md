...
- **Approvals**: enforced on the node host via `~/.zovsironclaw/exec-approvals.json`.
...
- The token is `gateway.auth.token` from the gateway config (`~/.zovsironclaw/zovsironclaw.json` on the gateway host).
...
- `--display-name` on `openclaw node run` / `openclaw node install` (persists in `~/.zovsironclaw/node.json` on the node).
...
Approvals live on the node host at `~/.zovsironclaw/exec-approvals.json`.
...
- On headless node host, `system.run` is gated by exec approvals (`~/.zovsironclaw/exec-approvals.json`).
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.zovsironclaw/node.json`.
- Exec approvals are enforced locally via `~/.zovsironclaw/exec-approvals.json`
