...
read_when:
  - You need detailed behavior for zovsironclaw onboard
...
This page is the full reference for `zovsironclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.zovsironclaw/zovsironclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `zovsironclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `zovsironclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.zovsironclaw/.env` so launchd can read it.
...
- OAuth credentials: `~/.zovsironclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.zovsironclaw/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`zovsironclaw onboard`](/cli/onboard)
