...
- **Session store** (chat history + routing state) under `~/.zovsironclaw/agents/<agentId>/sessions`.
...
available from `~/.zovsironclaw/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- Config: `~/.zovsironclaw/zovsironclaw.json` (or `OPENCLAW_CONFIG_PATH`)
- State dir: `~/.openclaw` (or `OPENCLAW_STATE_DIR`)
- Workspace: `~/.zovsironclaw/workspace` (or `~/.zovsironclaw/workspace-<agentId>`)
- Agent dir: `~/.zovsironclaw/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.zovsironclaw/agents/<agentId>/sessions`
...
- Workspace defaults to `~/.zovsironclaw/workspace` (or `~/.zovsironclaw/workspace-<profile>` when `OPENCLAW_PROFILE` is set).
- State defaults to `~/.zovsironclaw/agents/main/agent`.
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.zovsironclaw/credentials/whatsapp/personal
