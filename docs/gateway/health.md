...
- Creds on disk: `ls -l ~/.zovsironclaw/credentials/whatsapp/<accountId>/creds.json` (mtime should be recent).
- Session store: `ls -l ~/.zovsironclaw/agents/<agentId>/sessions/sessions.json` (path can be overridden in config). Count and recent recipients are surfaced via `status`.
...
- Gateway unreachable → start it: `zovsironclaw gateway --port 18789` (use `--force` if the port is busy).
