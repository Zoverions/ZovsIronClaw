...
- 从 `~/.zovsironclaw/dns/<domain>.db` 提供你选择的域名服务（示例：`openclaw.internal.`）
...
- 在 `~/.zovsironclaw/zovsironclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
- `~/.zovsironclaw/zovsironclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
