...
- 复制**状态目录**（`$OPENCLAW_STATE_DIR`，默认：`~/.zovsironclaw/`）— 这包括配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.zovsironclaw/workspace/`）— 这包括你的智能体文件（记忆、提示等）。
...
- Gateway 网关配置（`zovsironclaw.json`）
...
在这个阶段，如果新手引导创建了一个新的 `~/.zovsironclaw/` 也没关系 — 你将在下一步覆盖它。

### 步骤 2 — 将状态目录 + 工作区复制到新机器

复制**两者**：

- `$OPENCLAW_STATE_DIR`（默认 `~/.zovsironclaw/`）
- 你的工作区（默认 `~/.zovsironclaw/workspace/`）
...
### 陷阱：只复制 `zovsironclaw.json`

`zovsironclaw.json` 是不够的。许多提供商在以下位置存储状态：
