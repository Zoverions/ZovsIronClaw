...
**首选**的更新路径是重新运行网站上的安装程序。它会检测现有安装、原地升级，并在需要时运行 `zovsironclaw doctor`。
...
- 快照你的定制内容：
  - 配置：`~/.zovsironclaw/zovsironclaw.json`
  - 凭证：`~/.zovsironclaw/credentials/`
  - 工作区：`~/.zovsironclaw/workspace`
...
- 如果你的 Gateway 网关作为服务运行，`zovsironclaw gateway restart` 优于杀死 PID。
...
- 安装依赖、构建、构建控制 UI，并运行 `zovsironclaw doctor`。
...
- 在全局和 git 安装之间切换很容易：安装另一种方式，然后运行 `zovsironclaw doctor` 以便将 Gateway 网关服务入口点重写为当前安装。

## 始终运行：`zovsironclaw doctor`

Doctor 是"安全更新"命令。它故意很无聊：修复 + 迁移 + 警告。

注意：如果你是**源码安装**（git checkout），`zovsironclaw doctor` 会提供先运行 `openclaw update`。
...
- Windows（WSL2）：`systemctl --user restart openclaw-gateway[-<profile>].service`
  - `launchctl`/`systemctl` 仅在服务已安装时有效；否则运行 `zovsironclaw gateway install`。
...
- 再次运行 `zovsironclaw doctor` 并仔细阅读输出（它通常会告诉你修复方法）。
