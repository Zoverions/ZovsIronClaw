...
- 如果是升级现有安装：运行 `zovsironclaw doctor --non-interactive`（尽力执行）。
- 对于 git 安装：安装/更新后运行 `zovsironclaw doctor --non-interactive`（尽力执行）。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g openclaw@latest`
  - `git`：克隆/构建源码检出并安装包装脚本
- 在升级和 git 安装时运行 `zovsironclaw doctor --non-interactive`（尽力执行）。
