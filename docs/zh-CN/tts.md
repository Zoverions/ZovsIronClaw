...
TTS 配置位于 `zovsironclaw.json` 中的 `messages.tts` 下。完整 schema 在 [Gateway 网关配置](/gateway/configuration)中。
...
斜杠命令将本地覆盖写入 `prefsPath`（默认：`~/.zovsironclaw/settings/tts.json`，可通过 `OPENCLAW_TTS_PREFS` 或 `messages.tts.prefsPath` 覆盖）。
