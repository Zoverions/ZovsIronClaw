...
7. 直接聊天：默认通过 `channels.discord.dm.policy` 进行安全保护（默认：`"pairing"`）。未知发送者会收到配对码（1 小时后过期）；通过 `zovsironclaw pairing approve discord <code>` 批准。
...
- 首先：运行 `zovsironclaw doctor` 和 `openclaw channels status --probe`（可操作的警告 + 快速审计）。
...
- 如果机器人卡住或受到速率限制，在确认没有其他进程拥有 Discord 会话后重启 Gateway 网关（`zovsironclaw gateway --force`）。
