...
- **批准**：通过 `~/.zovsironclaw/exec-approvals.json` 在节点主机上执行。
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.zovsironclaw/zovsironclaw.json`）。
...
- 在 `openclaw node run` / `openclaw node install` 上使用 `--display-name`（持久化在节点上的 `~/.zovsironclaw/node.json` 中）。
...
批准存储在节点主机的 `~/.zovsironclaw/exec-approvals.json` 中。
...
- 在无头节点主机上，`system.run` 受 exec 批准限制（`~/.zovsironclaw/exec-approvals.json`）。
...
- 节点主机将其节点 id、令牌、显示名称和 Gateway 网关连接信息存储在 `~/.zovsironclaw/node.json` 中。
- Exec 批准通过 `~/.zovsironclaw/exec-approvals.json` 在本地执行
