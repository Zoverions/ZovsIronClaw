...
这与 `~/.zovsironclaw/` 是分开的，后者存储配置、凭证和会话。
...
当启用沙箱隔离且 `workspaceAccess` 不是 `"rw"` 时，工具在 `~/.zovsironclaw/sandboxes` 下的沙箱工作区内操作，而不是你的主机工作区。
...
- 在 `~/.zovsironclaw/zovsironclaw.json` 中覆盖：
...
`zovsironclaw onboard`、`openclaw configure` 或 `openclaw setup` 将创建工作区并在缺失时填充引导文件。
...
`zovsironclaw doctor` 在检测到额外工作区目录时会发出警告。
...
这些位于 `~/.zovsironclaw/` 下，不应提交到工作区仓库：

- `~/.zovsironclaw/zovsironclaw.json`（配置）
- `~/.zovsironclaw/credentials/`（OAuth token、API 密钥）
- `~/.zovsironclaw/agents/<agentId>/sessions/`（会话记录 + 元数据）
- `~/.zovsironclaw/skills/`（托管的 Skills）
...
如果你必须存储敏感引用，请使用占位符并将真正的密钥保存在其他地方（密码管理器、环境变量或 `~/.zovsironclaw/`）。
...
1. 将仓库克隆到所需路径（默认 `~/.zovsironclaw/workspace`）。
2. 在 `~/.zovsironclaw/zovsironclaw.json` 中将 `agents.defaults.workspace` 设置为该路径。
3. 运行 `openclaw setup --workspace <path>` 来填充任何缺失的文件。
4. 如果你需要会话，请单独从旧机器复制 `~/.zovsironclaw/agents/<agentId>/sessions/`。
