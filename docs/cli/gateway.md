---
summary: "OpenClaw Gateway CLI (`zovsironclaw gateway`) — run, query, and discover gateways"
read_when:
...
Subcommands in this page live under `zovsironclaw gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.zovsironclaw/zovsironclaw.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
```bash
zovsironclaw gateway health --url ws://127.0.0.1:18789
```
...
zovsironclaw gateway status --json
```
...
zovsironclaw gateway probe --json
```
...
```bash
zovsironclaw gateway probe --ssh user@gateway-host
```
...
```bash
zovsironclaw gateway call status
zovsironclaw gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
```bash
zovsironclaw gateway discover --timeout 4000
zovsironclaw gateway discover --json | jq '.beacons[].wsUrl'
```
