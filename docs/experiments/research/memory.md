...
read_when:
  - Designing workspace memory (~/.zovsironclaw/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.zovsironclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.zovsironclaw/workspace` as canonical human-readable memory.

Suggested workspace layout:

```
~/.zovsironclaw/workspace/
...
```
~/.zovsironclaw/workspace/.memory/index.sqlite
```
...
Pragmatic take for `~/.zovsironclaw/workspace`:
