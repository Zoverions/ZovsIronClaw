...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.zovsironclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.zovsironclaw/node.json` (node id + pairing token).
...
Path: `~/.zovsironclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.zovsironclaw/exec-approvals.sock",
...
- Unix socket at `~/.zovsironclaw/exec-approvals.sock` (0600).
