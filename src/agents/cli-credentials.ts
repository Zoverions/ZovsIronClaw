...
function readCodexKeychainCredentials(options?: {
  platform?: NodeJS.Platform;
  execSync?: ExecFileSyncFn;
}): CodexCliCredential | null {
  const platform = options?.platform ?? process.platform;
  if (platform !== "darwin") {
    return null;
  }
  const execFileSyncImpl = options?.execSync ?? execFileSync;
...
export function writeClaudeCliKeychainCredentials(
  ...
  options?: { execSync?: ExecFileSyncFn },
): boolean {
  const execFileSyncImpl = options?.execSync ?? execFileSync;
  try {
    ...
    const existingData = JSON.parse(existingResult.toString().trim());
