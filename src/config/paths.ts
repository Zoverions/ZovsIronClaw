...
const NEW_STATE_DIRNAME = ".zovsironclaw";
const CONFIG_FILENAME = "zovsironclaw.json";
 ...
 * Default: ~/.zovsironclaw/zovsironclaw.json (or $OPENCLAW_STATE_DIR/zovsironclaw.json)
 */
 ...
 * Default: os.tmpdir()/zovsironclaw-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `zovsironclaw-${uid}` : "zovsironclaw";
